...
local function cics_info( host, port, commands, user, pass, cemt, trans )
  ...
  --  if tn:find("Sign-on") then
  --    stdnse.debug(1,"Found CESL/CESN 'Signon' sending PF3")
  --    tn:send_pf(3)
  --    tn:get_all_data()
  --  end
  --  tn:get_all_data()
  --  stdnse.debug(1,"Clearing the Screen")
  --  tn:send_clear()
  --  tn:get_all_data()
  --  tn:get_screen_debug(2)
  --  count = count + 1
  ...
  if user and pass then -- We're doing authenticated CICS testing now baby!
    stdnse.verbose(1,'Logging in with %s / %s for auth testing', user, pass)
    --tn:send_clear()
    --tn:get_all_data()
    --tn:get_screen_debug(2)
    --tn:send_cursor('CESN')
    ...
    --local pass_loc = {fields[2][1],pass}   -- This is the 'Password:' field ([2] is a group ID)
    stdnse.verbose('Trying CICS: %s : %s', user, pass)
    --tn:send_locations({user_loc,pass_loc})
    tn:send_locations({{669,user},{749,pass}})
    tn:get_all_data()
    stdnse.debug(1,"Screen Received for User ID: %s / %s", user, pass)
    tn:get_screen_debug(1)
    count = 1
    while not (tn:find('DFHCE3549') or  tn:find('ROYAL BANK OF SCOTLAND CICS')) and count < 6 do
        tn:get_all_data(1000) -- loop for 6 seconds
        tn:get_screen_debug(1)
        count = count + 1
    end
    if not (tn:find('DFHCE3549') or tn:find('ROYAL BANK OF SCOTLAND CICS'))   then
  ...
  --tn:get_all_data()
  --tn:get_screen_debug(2)
  --stdnse.debug(1,"Sending 'Clear Screen'")
  --tn:send_clear()
  --tn:get_all_data()
  --tn:get_screen_debug(1)
  stdnse.verbose(2,"Sending 'CEMT INQUIRE SYSTEM'")
  tn:send_location(81,'CEMT INQUIRE SYSTEM')
  tn:get_all_data()
  tn:get_screen_debug(2)
  ...
  tn:get_all_data()
  tn:get_screen_debug(2)
  -- For some reason sending clear doesn't really work
  --stdnse.debug(1,"Sending 'Clear Screen'")
  --tn:send_clear()
  --tn:get_all_data()
  --tn:get_screen_debug(1)
  stdnse.verbose(2,"Sending 'CEMT INQUIRE DSNAME'")
  tn:send_location(81,'CEMT INQUIRE DSNAME')      
  tn:get_all_data()
  tn:get_screen_debug(2)
  ...
  tn:send_location(81,'CEMT INQUIRE LIBRARY')
  tn:get_all_data()
  tn:get_screen_debug(2)
  ...
  tn:send_location(81,'CEMT INQUIRE TASK')
  tn:get_all_data()
  tn:get_screen_debug(2)
  ...
  tn:send_location(81,'CEMT INQUIRE TRANSACTION('.. trans ..') en')
  tn:get_all_data()
  tn:get_screen_debug(2)
