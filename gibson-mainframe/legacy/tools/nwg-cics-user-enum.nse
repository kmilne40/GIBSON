...
Driver = {
  ...
  login = function (self, user, pass) -- pass is actually the UserID we want to try
    ...
    local loop = 1
    local err
    stdnse.debug(1,"Getting to CICS")
    ...
    if not (self.tn3270:find('SIGN ON TO CICS') or  self.tn3270:find("CICS") ) then -- this part was altered to expand the detection
    ...
    if not (self.tn3270:find('Type your userid and password') or self.tn3270:find('Userid')) then -- this part got altered to expand the detection
    ...
    self.tn3270:get_screen_debug(2)
    -- 669 is the location of userid 749 is the location of the password
    self.tn3270:send_locations({{669,pass},{749,"1234"}}) -- this was altered, as our testing mainframe's CICS requires a password to be sent as well
    self.tn3270:get_all_data()
    stdnse.debug(1,"Screen Received for User ID: %s", pass)
    self.tn3270:get_screen_debug(1)
    if self.tn3270:find('TSS7145E') or
       self.tn3270:find('ACF01004') or
       self.tn3270:find('Invalid userid.') or -- this condition was added to expand the detection
    ...
    elseif self.tn3270:find('TSS7102E') or
           ...
           self.tn3270:find('Invalid password.') or -- this condition was added to expand the detection
      ...
      return true, creds.Account:new("CICS User: ".. string.upper(pass),'Empty or inconclusive screen received, likely but not certainly valid, try again', creds.State.VALID)
    end
...
local function cics_test( host, port, commands, transaction )
  ...
  if tn:find('Type your userid and password') or (tn:find('Userid') and tn:find('Password')) then --TODO check it was here
    stdnse.verbose(1,"At CICS Login Transaction")
    tn:disconnect()
    return true
  end
  ...
  stdnse.debug(1,"Sending %s", transaction)
  ...
  if tn:find('SIGN ON TO CICS') or tn:find("Signon to CICS") then
      stdnse.verbose(1,"At CICS Login Transaction (%s)", transaction)
      tn:disconnect()
      return true
  end
  tn:disconnect()
