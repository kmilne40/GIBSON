...
Driver = {
  ...
  login = function (self, user, pass) -- pass is actually the username we want to try
    ...
    local type = "applid"
    local seenIncorrect = 0
    local incorrectValue = 0
    local threshold = 78 -- change sensitivity
    ...
    if (self.tn3270:find('UNABLE TO ESTABLISH SESSION')  or -- thanks goes to Dominic White for creating these
        ...
        self.tn3270:find('UNABLE TO CONNECT TO THE REQUESTED APPLICATION') 
	then 
		if seenIncorrect == 0 then 
			seenIncorrect = 1
			incorrectValue = screen_diff(previous_screen, current_screen)
		end
      		-- Looks like an invalid APPLID.
      		stdnse.verbose(1,'Invalid Application ID: %s ',string.upper(pass) )
      		return false,  brute.Error:new( "Invalid VTAM Application ID" )
	else

        if screen_diff(previous_screen, current_screen) ==  incorrectValue and incorrectValue ~= 0 then
      -- Looks like an invalid APPLID.
      stdnse.verbose(1,'Invalid Application ID: %s ',string.upper(pass) )
      return false,  brute.Error:new( "Invalid VTAM Application ID" )
      end
    
      stdnse.verbose(1,"Valid Application ID: %s",string.upper(pass))
      if path ~= nil then
